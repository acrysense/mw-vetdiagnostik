...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (eventsCards.length > 0) {
    ...
    gsap.utils.toArray(eventsCards).forEach(function (item) {
      gsap.to(item, {
        y: 0,
        opacity: '1',
        filter: 'blur(0px)',
  ...
  if (cooperationDecor.length > 0) {
    gsap.utils.toArray(cooperationDecor).forEach(function (item) {
      gsap.to(item, {
        y: -30,
        ease: "none",
        scrollTrigger: {
  ...
  if (cooperationItem.length > 0) {
    ...
    gsap.utils.toArray(cooperationItem).forEach(function (item) {
      gsap.to(item, {
        y: 0,
        opacity: '1',
        filter: 'blur(0px)',
  ...
  if (researchCard.length > 0) {
    ...
    gsap.utils.toArray(researchCard).forEach(function (item) {
      gsap.to(item, {
        y: 0,
        filter: 'blur(0px)',
        scale: '1',
  ...
  if (articleshCard.length > 0) {
    ...
    gsap.utils.toArray(articleshCard).forEach(function (item) {
      gsap.to(item, {
        y: 0,
        opacity: '1',
        filter: 'blur(0px)',
  ...
  window.addEventListener('load', function () {
    if (aboutWorldsItem.length > 0) {
      ...
      // Немного задерживаем обновление, чтобы все размеры точно были рассчитаны
      setTimeout(function () {
  ...
  // rgbeLoader.setPath('scope/');
  // rgbeLoader.load('123.hdr', (texture) => {
  // 	texture.mapping = THREE.EquirectangularReflectionMapping;
  // 	scene.environment = texture;
  // });

  // Используем стандартный загрузчик текстур:
  var textureLoader = new THREE.TextureLoader();
  textureLoader.setPath('scope/'); // Папка, где лежит PNG-файл
  textureLoader.load('123.png', function (texture) {
    // Если PNG подготовлен как equirectangular,
    // задаем mapping, чтобы Three.js правильно интерпретировал его для отражений.
    texture.mapping = THREE.EquirectangularReflectionMapping;

    // Назначаем текстуру как окружение для сцены (это влияет на отражения в PBR материалах)
    scene.environment = texture;
  });
  ...
  loader.load('123.glb', function (gltf) {
    ...
    // Настройка текстур (отладка)
    microscope.traverse(function (child) {
      if (child.isMesh && child.material) {
        if (child.material.map) {
          console.log("\u041C\u0435\u0448 ".concat(child.name, ": \u0431\u0430\u0437\u043E\u0432\u0430\u044F \u0442\u0435\u043A\u0441\u0442\u0443\u0440\u0430 \u043D\u0430\u0439\u0434\u0435\u043D\u0430"), child.material.map.image);
          child.material.map.encoding = THREE.sRGBEncoding;
          child.material.needsUpdate = true;
        } else {
          console.warn("\u041C\u0435\u0448 ".concat(child.name, ": \u0431\u0430\u0437\u043E\u0432\u0430\u044F \u0442\u0435\u043A\u0441\u0442\u0443\u0440\u0430 \u043E\u0442\u0441\u0443\u0442\u0441\u0442\u0432\u0443\u0435\u0442"));
        }
        if (child.geometry && !child.geometry.attributes.uv) {
          console.warn("\u041C\u0435\u0448 ".concat(child.name, " \u043D\u0435 \u0441\u043E\u0434\u0435\u0440\u0436\u0438\u0442 UV-\u043A\u043E\u043E\u0440\u0434\u0438\u043D\u0430\u0442!"));
        }
    ...
    // Определяем камеру для рендера (если экспортирована)
    ...
    var finalObjPos = new THREE.Vector3(13, -1.1, 0.55);
    var finalObjEuler = new THREE.Euler(THREE.MathUtils.degToRad(21), THREE.MathUtils.degToRad(38), THREE.MathUtils.degToRad(-17), "XYZ");
    var finalObjQuat = new THREE.Quaternion().setFromEuler(finalObjEuler);
    console.log("Начальная позиция объекта:", initialObjPos);
    console.log("Конечная позиция объекта:", finalObjPos);
    console.log("Начальный кватернион:", initialObjQuat);
    console.log("Конечный кватернион:", finalObjQuat);
    gsap.registerPlugin(ScrollTrigger);
    ScrollTrigger.create({
      ...
      onUpdate: function onUpdate(self) {
        microscope.position.lerpVectors(initialObjPos, finalObjPos, self.progress);
        microscope.quaternion.slerpQuaternions(initialObjQuat, finalObjQuat, self.progress);
      }
    });
    gsap.to(".main", {
      opacity: 0,
    ...
    gsap.utils.toArray('.advantages__item').forEach(function (item) {
      gsap.to(item, {
        opacity: 1,
        ease: 'none',
        scrollTrigger: {
