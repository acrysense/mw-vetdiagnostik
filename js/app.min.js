...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (aboutWorlds) {
    // Функция мгновенного обновления состояния слайдов:
    // – Активный слайд: y = 0, opacity = 1, без блюра.
    // – Все слайды выше активного: смещаются вверх (y = -50) и скрываются (opacity = 0, блюр 5px).
    // – Все слайды ниже активного: располагаются ниже с отступом 50px за каждым, opacity = 0, блюр 5px.
    ...
    }; // Инициализация – первый слайд активен
    var slides = gsap.utils.toArray('.about-worlds__item');
    var numSlides = slides.length;
    var slideDistance = 100; // каждые 100px скролла переключение
    var totalScrollDistance = (numSlides - 1) * slideDistance;
    updateSlides(0);
    ScrollTrigger.create({
      ...
      pin: true,
      // секция фиксируется на время скролла
      scrub: true,
      // синхронизация анимации с прокруткой
      snap: {
        // Snap округляет прогресс так, что каждые 100px (1/(numSlides-1)) происходит переключение
        snapTo: 1 / (numSlides - 1),
        duration: 0,
        // мгновенное переключение без tweening
        delay: 0,
        ease: "none"
      },
      onUpdate: function onUpdate(self) {
        // Вычисляем новый индекс активного слайда по прогрессу (от 0 до numSlides - 1)
