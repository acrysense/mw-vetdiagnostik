...
onReady(function () {
  ...
  // 4) При изменении размеров/контента обновляем расчёты
  window.addEventListener('resize', function () {
    return ScrollTrigger.refresh();
  });
  setTimeout(function () {
    return ScrollTrigger.refresh();
  }, 0); // на всякий случай после первого рендера
  ...
  if (aboutCards.length > 0) {
    ...
    gsap.utils.toArray(aboutCards).forEach(function (item, i) {
      gsap.to(item, {
        y: 0,
        opacity: '1',
        filter: 'blur(0px)',
  ...
  if (header && document.querySelector('.about')) {
    ScrollTrigger.create({
      trigger: '.about',
      start: 'top top',
      onEnter: function onEnter() {
        return header && header.classList.add('is--hidden');
      },
      onLeaveBack: function onLeaveBack() {
        return header && header.classList.remove('is--hidden');
      }
    });
  }
  ...
  var LENIS_PREVENT_ATTRS = ['data-lenis-prevent', 'data-lenis-prevent-wheel', 'data-lenis-prevent-touch'];
  function setLenisPrevent(el) {
    var on = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
    if (!el) return;
    LENIS_PREVENT_ATTRS.forEach(function (attr) {
      return on ? el.setAttribute(attr, '') : el.removeAttribute(attr);
    });
  }
  ...
  function feedbackOpenModal(targetTab) {
    if (feedbackModal) {
      ...
      var wrap = feedbackModal.querySelector('.feedback-modal__wrap');
      setLenisPrevent(wrap, true);
      bodyFixScroll(1);
    }
  }
  ...
  if (popupImage && popupImageTrigger && popupImageClose) {
    ...
    popupImageTrigger.forEach(function (link) {
      link.addEventListener('click', function (event) {
        ...
        var wrap = popupImage.querySelector('.popup-image__wrap');
        setLenisPrevent(wrap, true);
        bodyFixScroll(1);
      });
    });
  ...
  // main bg
  var mainBG = document.querySelector('.main__bg');
  if (mainBG) {
  ...
  ;
  (function () {
    var smRoot = document.querySelector('.about__slider-sm');
    var lgRoot = document.querySelector('.about__slider-lg');
    if (!smRoot || !lgRoot) return;
    var smEl = smRoot.querySelector('.swiper');
    var lgEl = lgRoot.querySelector('.swiper');
    if (!smEl || !lgEl) return;
    var isMobile = window.matchMedia('(max-width: 743px)').matches;
    var lgSwiper = new Swiper(lgEl, {
      direction: 'vertical',
      slidesPerView: 'auto',
      speed: 700,
      ...
      navigation: {
        prevEl: lgRoot.querySelector('.about__nav-btn--prev'),
        nextEl: lgRoot.querySelector('.about__nav-btn--next')
      },
      pagination: {
        el: lgRoot.querySelector('.about__pagination--lg'),
        clickable: true
      }
    });
    var smSwiper = new Swiper(smEl, {
      direction: 'vertical',
      slidesPerView: 'auto',
      speed: 700,
      allowTouchMove: !isMobile,
      ...
      touchStartPreventDefault: false,
      navigation: {
        prevEl: smRoot.querySelector('.about__nav-btn--prev'),
        nextEl: smRoot.querySelector('.about__nav-btn--next')
      },
      pagination: {
        el: smRoot.querySelector('.about__pagination--sm'),
        clickable: true
      }
    });
    if (lgSwiper.controller && smSwiper.controller) {
      lgSwiper.controller.control = smSwiper;
      smSwiper.controller.control = lgSwiper;
    }
    smRoot.addEventListener('click', function (e) {
      var slide = e.target.closest('.swiper-slide');
      if (!slide) return;
      var index = Array.prototype.indexOf.call(slide.parentNode.children, slide);
      if (index < 0) return;
      lgSwiper.slideTo(index);
    });
  })();
});